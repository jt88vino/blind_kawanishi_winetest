...
export function sql(){const url=process.env.NEON_DATABASE_URL||process.env.DATABASE_URL;if(!url)throw Error('DATABASE_URL is not configured');return neon(url);}
