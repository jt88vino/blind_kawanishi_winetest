...
export async function POST(r:Request){if(!originOk(r))return json({error:'アクセスを確認できません'},403);try{
 const password=process.env.ADMIN_PASSWORD;if(!password||password.length<10||!process.env.SESSION_SECRET||process.env.SESSION_SECRET.length<32)return json({error:'管理者ログインはまだ設定されていません。'},503);
