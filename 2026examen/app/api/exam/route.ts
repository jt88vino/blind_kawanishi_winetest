import {choices,normalizeOptions} from '@/lib/options';
...
export async function POST(r:Request){try{
 ...
 if(b.action==='configure'){
  ...
  let questions:Question[];try{const old=new Map((c.questions as Question[]).map(x=>[x.id,x]));questions=b.questions.map((x:Question)=>{const o=old.get(x.id),options=normalizeOptions(x);const changed=o&&(o.type!==x.type||o.label!==x.label.trim()||JSON.stringify(normalizeOptions(o))!==JSON.stringify(options));return {id:changed?crypto.randomUUID():x.id,type:x.type,label:x.label.trim(),options}});}catch(e){return json({error:e instanceof Error?e.message:'選択肢が不正です。'},400);}
