...
import {EXAMS,WHITE,RED,COUNTRIES,YEARS,SPIRITS,type Exam,type Question} from '@/lib/exam';
...
export async function POST(r:Request){try{
 if(!originOk(r))return json({error:'アクセスを確認できません'},403);const raw=await r.text();if(raw.length>50000)return json({error:'データが大きすぎます'},413);let b:any;try{b=JSON.parse(raw)}catch{return json({error:'入力が不正です'},400)}if(!b||!Object.hasOwn(EXAMS,b.exam))return json({error:'試験区分が不正です'},400);const exam=b.exam as Exam;
 if(b.action==='configure'&&!await admin())return json({error:'管理者のみ操作できます'},403);
 ...
 if(c.revision!==b.revision)return json({error:'問題が変更されました。ページを再読み込みしてご確認ください。'},409);
 if(!Array.isArray(b.answers)||b.answers.length!==c.questions.length)return json({error:'すべての問題に回答してください。'},400);
 const answers=[];for(const item of c.questions as Question[]){const a=b.answers.find((x:any)=>x?.id===item.id)?.values;if(!a)return json({error:'すべての問題に回答してください。'},400);const valid=item.type==='spirit'?SPIRITS.includes(a.drink):COUNTRIES.includes(a.country)&&(item.type==='white'?WHITE:RED).concat(['その他','わからない']).includes(a.grape)&&YEARS.includes(a.year);if(!valid)return json({error:'すべての項目を選択してください。'},400);answers.push({...item,values:item.type==='spirit'?{drink:a.drink}:{country:a.country,grape:a.grape,year:a.year}});}
 // Row lock serializes an in-flight submission against a settings update.
 const inserted=await q.query(`WITH current_config AS (SELECT exam FROM bk_settings WHERE exam=$1 AND revision=$2 FOR SHARE) INSERT INTO bk_submissions(device,exam,answers) SELECT $3::uuid,$1,$4::jsonb FROM current_config ON CONFLICT(device) DO NOTHING RETURNING device`,[exam,b.revision,id,JSON.stringify(answers)]);
