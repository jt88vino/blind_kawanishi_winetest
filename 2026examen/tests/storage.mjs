...
try {
 ...
 assert.equal((await db.query(submission,['sommelier',1,id,JSON.stringify(answers)])).rows.length,1);
 assert.equal((await db.query(submission,['sommelier',1,id,JSON.stringify(answers)])).rows.length,0);
 assert.equal((await db.query(submission,['sommelier',0,crypto.randomUUID(),JSON.stringify(answers)])).rows.length,0);
